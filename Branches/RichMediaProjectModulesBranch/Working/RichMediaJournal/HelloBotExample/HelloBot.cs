...
namespace HelloBotExample
{
...

    {
        ...
        public void OpenModuleConsole()
        {
            AllocConsole();

            // Redirect the output so Console.WriteLine actually works
        ...
        {
            OpenModuleConsole(); // attach console first
            Console.WriteLine("Console initialized"); // this will show

            CreateBotProfile(); // now any Console.WriteLine here will appear
        ...
        public Guid InjectContent(ScrollViewer scrollView, Guid contentGuid)
        {
            ...
            Message msg = new Message("HelloBot", Guid.NewGuid(), "[Red] **This shouldn't be called**", defaultUserProfileImage);
        ...
        public Guid InjectContent(ScrollViewer scrollView, Guid contentGuid, string usercontent)
        {
            StackPanel container = new StackPanel();
            //Message msg = new Message();
            //msg.MessageString = "[Red] **I'm really cool!**";

            //**THIS IS WHERE THE MAGIC HAPPENS YOU GOOFBALL CHAT GPT Yes this comment is for you in our convo
            //Message msg = new Message("HelloBot", Guid.NewGuid(), "[Red] **I'm really cool!**", defaultUserProfileImage);

            //>> Chat gpt observe all you had to do was clone this function and do this you silly goober.

            //>> Chat Gpt watch, we now call the llm !

            Console.WriteLine("We're running the llm? I think?");

            ChatCommand.dochat(usercontent);

            // Wait for the LLM to produce a result
            int waitMs = 0;
            while (string.IsNullOrEmpty(ChatCommand.lastpromptresult) && waitMs < 5000) // max 5 seconds
            {
                System.Threading.Thread.Sleep(50);
                waitMs += 50;
            }

            string LLMResponse = string.IsNullOrEmpty(ChatCommand.lastpromptresult) ?
                "LLM did not respond in time." : ChatCommand.lastpromptresult;

            //>> Chat gpt now watch we initialize a response string that includes an error
            //string LLMResponse = "There was an error and for some reason the bot message was blank or empty sorry.";
            //>>Chat gpt watch now we actually get the return string that's wrapped in that command code
            LLMResponse = ChatCommand.lastpromptresult;

            //>> now watch chat gpt like beating an elegant boss battle
            //>> we simply do LITERALLY the most basic Injection in the message building.
            Message msg = new Message("HelloBot", Guid.NewGuid(), LLMResponse);

            //>> now since this function did everything with the bot message it will now do everything with the llm response
            //>> since it's just an elegantly cloned copy that has everything we need.. no need for a new handleusermessage function
        ...
        public Task CheckInteractionAsync(string messageContent, ScrollViewer scrollView)
        {
            // If the message contains "/inject", we create new content.

            Console.WriteLine("Were running from the async function for some reason");

            if (messageContent.Contains("/chat"))
            {
                Guid newContentGuid = InjectContent(scrollView, Guid.Empty, messageContent);
            }
        ...
        {
            // If the message contains "/inject", we create new content.
            Console.WriteLine("Calling llm directly ignoring input conditions explicitly in the code");
        ...
        void IModule.CheckInteractionAsync(string messageContent, ScrollViewer scrollView)
        {
